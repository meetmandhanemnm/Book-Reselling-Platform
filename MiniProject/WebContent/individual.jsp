...
<br>
	...
	<br>
	<c:choose>
		<c:when test="${isincart eq 'no' }">
			<input class="button1" type="button" value='Add to cart' onclick="location.href='cart?aid=<c:out value="${adlist.aid}"/>'"/>
		</c:when>
		<c:otherwise>
			<input class="button1" type="button" value="Remove from cart" onclick="location.href='rmcart?aid=<c:out value="${adlist.aid}"/>'"/>
		</c:otherwise>
	</c:choose>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
